...
def calculate_bearing(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """Calculates forward azimuth bearing between two points (0 - 360)."""
...
def check_geofence(
    ...
    """Evaluates whether a coordinate is inside a geofence radius."""
...
def calculate_nearest_intercept(target_lat: float, target_lng: float, candidates: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Identifies the closest safe officer to intercept breached personnel."""
    if not candidates:
        return None
    ...
    for cand in candidates:
        ...
        d = calculate_distance_meters(target_lat, target_lng, c_lat, c_lng)
        if d < min_dist:
            min_dist = d
            best_candidate = cand
    ...
    # Estimated ETA assuming average tactical response vehicle speed (45 km/h = 12.5 m/s)
    eta_sec = min_dist / 12.5
